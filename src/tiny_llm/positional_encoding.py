...
class RoPE:
    ...
    def __call__(
        ...
        # Split x into even/odd channels or half-and-half
        ...
        if self.traditional:
            ...
            # Stack and flatten interleaved dimensions
            return mx.stack((o1, o2), axis=-1).reshape(N, L, H, D)
        else:
            # Qwen2 / Neox-style
            x1, x2 = mx.split(x, 2, axis=-1)  # both (N, L, H, D/2)
    
            o1 = x1 * cos - x2 * sin
            o2 = x2 * cos + x1 * sin
    
            # Concatenate the rotated halves along last dim
            return mx.concatenate((o1, o2), axis=-1)  # (N, L, H, D)
